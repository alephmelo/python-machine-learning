...
class RedeNeural:
    def __init__(self, nos_entrada, nos_ocultos, nos_saida):
        ...
        # cria uma matriz, preenchida com uns, de uma linha pela quantidade de
        # nos
    ...
    def fase_forward(self, entradas):
        ...
        # Note que as saidas dos neuronios da camada 
        # oculta fazem o papel de entrada
    ...
    def fase_backward(self, saidas_desejadas):
        ...
        for i in range(self.nos_saida):
            ...
            output_deltas[i] = derivada_funcao_ativacao(
                self.ativacao_saida[i]) * erro
        ...
        for i in range(self.nos_ocultos):
            ...
            hidden_deltas[i] = derivada_funcao_ativacao(
                self.ativacao_ocultos[i]) * erro

        # a partir da ultima camada ate a camada de entrada
        # os nos da camada atual ajustam seus pesos de forma a reduzir seus
        # erros
        for i in range(self.nos_ocultos):
            for j in range(self.nos_saida):
                change = output_deltas[j] * self.ativacao_ocultos[i]
                self.wo[i][j] = self.wo[i][
                    j] + (self.taxa_aprendizado * change) + (self.momentum * self.co[i][j])
                self.co[i][j] = change
        ...
        for i in range(self.nos_entrada):
            for j in range(self.nos_ocultos):
                ...
                self.wi[i][j] = self.wi[i][
                    j] + (self.taxa_aprendizado * change) + (self.momentum * self.ci[i][j])
                self.ci[i][j] = change
        ...
        for i in range(len(saidas_desejadas)):
            erro = erro + 0.5 * \
                (saidas_desejadas[i] - self.ativacao_saida[i]) ** 2
        return erro
    ...
    def test(self, entradas_saidas):
        for p in entradas_saidas:
            ...
            print(
                "Entradas: " + str(p[0]) + ' - Saída encontrada/fase forward: ' + str(array[0]))
